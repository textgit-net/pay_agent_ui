...
import {channelOutTest, ChannelOutTestRequest,AccountTypeEnum,getAccountTypeEnumText,getALLChannelOutList } from "~/api/transdeposit/index.ts";
import { getTransOrderStatusText} from '~/api/transdeposit/order.ts'
...
const fromData=reactive<ChannelOutTestRequest>({
  //  isWebCashier:true,
  title: '',
  remark: '',
   amount:0.1,
   channelId: null,
   accountType: AccountTypeEnum.aliPay
  //  payMode: null
})
...
const createOrder=()=>{
  formRef.value
      ...
      .then(() => {
        ...
        channelOutTest(unref(fromData)).then(res=>{
...
const fetchALLChannelList = async () => {
  let res = await getALLChannelOutList(allChannelListRequest.value)
...
const disabledBtn = computed(() => {
  let {amount,accountNo,accountName,idCardNo,accountType,channelId} = fromData
  if (accountType == AccountTypeEnum.aliPay) {
    return !amount || !accountNo || !accountName || !channelId
  } else if (accountType == AccountTypeEnum.bankAccount) {
    return !amount || !accountNo || !accountName || !channelId || !idCardNo
  }
    ...
    <a-flex vertical justify="center" align="center" style="width: 800px;padding: 15px;border-radius: 4px" >
      ...
      <a-card style="width: 100%;margin-bottom: 10px;">
        <a-flex align="center" justify="flex-start">
          ...
          <a-page-header title="进件测试" class="site-page-header" sub-title="" style="padding: 0;">
      ...
      <a-card style="width: 100%;padding: 15px;">
        ...
        <a-form ref="formRef"  v-show="state.current==0" class="mt-4" style="width: 80%" :model="fromData"  layout="vertical"   >
          ...
          <a-form-item label="账户类型" name="accountType" :rules="{required:true,message:'请选择账户类型',trigger: 'change'}">
            <a-radio-group v-model:value="fromData.accountType">
              <a-radio :value="AccountTypeEnum.aliPay">{{getAccountTypeEnumText(AccountTypeEnum.aliPay)}}</a-radio>
              <a-radio :value="AccountTypeEnum.bankAccount">{{getAccountTypeEnumText(AccountTypeEnum.bankAccount)}}</a-radio>
            </a-radio-group>
            
          </a-form-item>

          <a-form-item  label="账户姓名"  name="accountName" :rules="{required:true,message:'请输入账户姓名',trigger: 'change'}">
            <a-input  placeholder="请输入账户姓名" v-model:value="fromData.accountName" style="width: 100%;"></a-input>
          </a-form-item>
          <a-form-item label="账户" name="accountNo" :rules="{required:true,message:'请输入账户',trigger: 'change'}">
            <a-input  placeholder="请输入账户"  v-model:value="fromData.accountNo" style="width: 100%;"></a-input>
          </a-form-item>
          <a-form-item v-if="fromData.accountType == AccountTypeEnum.bankAccount" label="身份证号" name="idCardNo" :rules="{required:true,message:'请输入身份证号',trigger: 'change'}">
            <a-input  placeholder="请输入身份证号"  v-model:value="fromData.idCardNo" style="width: 100%;"></a-input>
          </a-form-item>
          ...
          <a-flex vertical v-if="isExtend" v-show="fromData.channelId!=null">
            <a-form-item label="商品标题">
              <a-input  placeholder="请输入商品标题" v-model:value="fromData.title" style="width: 100%;"></a-input>
            </a-form-item>
        ...
        <a-flex  v-if="state.current==1" vertical justify="center" align="center" style="padding: 60px" :gap="10">
          ...
          <!-- <qrcode-vue
            ...
            foreground='#fff'
          /> -->
          {{ getTransOrderStatusText(orderInfo["status"]) }}
          <a-typography-text type="secondary">订单号：{{ orderInfo["orderId"] }}</a-typography-text>
        </a-flex>
        ...
        <a-flex class="mt-4"  justify="center" align="center" :gap="10" >
          <a-button @click="createOrder" :loading="state.isCreateLoading" :disabled="disabledBtn" v-if="state.current==0" type="primary" class="mt-0" size="large" style="width: 250px">下一步</a-button>
          <a-button @click="state.current=0" v-if="state.current==1" class="mt-0" size="large" style="width: 250px">上一步</a-button>
          <a-button @click="doCheckPayOrderStatus"  type="primary" v-if="state.current==1" class="mt-0" size="large" style="width: 250px">查看订单</a-button>
        </a-flex>
      </a-card>
