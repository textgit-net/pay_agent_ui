      ...
      <a-form ref="formRef" style="width: 800px" :model="formData"  :labelCol="{style: 'width: 140px; display: inline-block;'}">
        <a-flex vertical :gap="10">
          ...
          <a-card >
            ...
            <a-form-item label="代理名称：" name="name"  class="mt-5" :rules="[{required:true,message:'请输入代理名称'}]">
              <a-flex style="flex: 1" vertical>
                <a-input v-model:value="formData.name" placeholder="请输入代理名称"  style="width: 320px"/>
            ...
            <a-form-item label="代理简称：" name="shortName"  class="mt-5" :rules="[{required:false,message:'请输入代理简称'}]">
              <a-flex style="flex: 1" vertical>
                <a-input v-model:value="formData.shortName" placeholder="请输入代理简称"  style="width: 320px"/>
                <a-typography-text type="secondary">为了更好的管理请规范输入</a-typography-text>
              </a-flex>
            </a-form-item>
            ...
            <a-form-item label="联系账号：" name="contactNumber" class="mt-5" :rules="[{required:true,message:'请输入联系账号'}]">
              <a-input v-model:value="formData.contactNumber" placeholder="请输入联系账号"  style="width: 320px"></a-input>
            </a-form-item>
          ...
          <a-card >
            ...
            <a-form-item label="登录密码：" name="password" v-show="formData.id==null" class="mt-5" :rules="[{required:formData.id==null,message:'请输入登录密码'}]">
              <a-flex :gap="10">
                <a-flex style="flex: 1" vertical>
                  <a-input v-model:value="formData.password" placeholder="请输入登录密码"  style="width: 320px"></a-input>
