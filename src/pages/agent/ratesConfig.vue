...
const productRateList = ref<ProductItem[]>([])
...
const getInfo=async (id:string)=>{
  ...
  info.value = data
  productRateList.value = info.value.products?? []
  productRateList.value = productRateList.value.filter(i => i.isEnable)
  state.isLoading=false
  
}
const handleAddProduct = () => {
  state.isShowRate=true
  productRateList.value = JSON.parse(JSON.stringify(productRateList.value))
}



const onSuccess = async (products: ProductItem[]) => {
  try {
    let res = await changeAgentProducts(id as string, products)
...
<template>
  <a-card :loading="state.isLoading" :bodyStyle="{padding: '30px 20px '}">
    ...
    <a-flex vertical>
      ...
      <a-table :loading="state.isLoading" bordered size="small" :columns="columns" :data-source="productRateList" class="mt-4" :pagination="false">
        <!-- <template #emptyText>
          <a-empty></a-empty>
