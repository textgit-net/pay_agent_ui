...
const state=reactive({
  isShowData: localStorage.getItem('ChannelOrderListIsShowData')? eval(localStorage.getItem('ChannelOrderListIsShowData')): false,
...
const initDataValue = (): OrderStatisticsResponse => {
    if (localStorage.getItem('ChannelOrderStatisticsData')) {
      return JSON.parse(localStorage.getItem('ChannelOrderStatisticsData'))
    }
...
watch(() => state.isShowData, (val) => {
  localStorage.setItem('ChannelOrderListIsShowData', val)
  if (val) {
    getStatisticsData()
    console.log(val)
  } else {
    localStorage.removeItem('ChannelOrderListIsShowData')
    localStorage.removeItem('ChannelOrderStatisticsData')
  }
})
const getStatisticsData = async () => {
    ...
    try {
      let res = await getOrderStatisticsData({
        type: OrderStatisticsEnum.channel,
        params: id as string
      })
      localStorage.setItem('ChannelOrderStatisticsData', JSON.stringify(res.data))
